...
for run_time in xrange(run_times):
    ...
    print('Build model...')
    model = Sequential()
    model.add(GRU(input_dim=len(chars), output_dim=512, return_sequences=False))
    ...
    record_file = FileIO()
    record_file.save_pickle(losses_record, "single_gru_%d_adam_sigmoid_titan_losses"%run_time )
    record_file.save_pickle(times_record, "single_gru_%d_adam_sigmoid_titan_times"%run_time )
