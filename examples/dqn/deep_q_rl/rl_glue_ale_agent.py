...
class DeepQNetwork(Sequential):
    ...
    def compile(self, optimizer, loss, class_mode="categorical", theano_mode=None):
        super(DeepQNetwork, self).compile(optimizer, loss, class_mode, theano_mode)
        self._compute_q_vals = self._predict
...
class NeuralAgent(Agent):
    ...
    def _init_network(self):
        ...
        model = DeepQNetwork(num_actions=self.num_actions)
        model.add(Convolution2D(32, 4, 8, 8, border_mode='full'))
        model.add(Activation('relu'))

        model.add(Convolution2D(64, 4, 4, 4))
        model.add(Activation('relu'))

        model.add(Convolution2D(64, 4, 3, 3, border_mode='full'))
        model.add(Activation('relu'))

        model.add(Flatten())
        model.add(Dense(64*3*3, 512, init='normal'))
        model.add(Activation('relu'))
