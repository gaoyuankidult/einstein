...
def testincsfa(animate=False):
    try:        
        ...
        raise Exception('Intall MDP toolkit for check/test')
    
    iterval = 20

    times_steps = 200
    t = np.linspace(0, 4 * np.pi, times_steps)
    x1 = np.sin(t) + np.power(np.cos(11 * t), 2) + np.random.randn(times_steps) * 0.1
    x2 = np.cos(11 * t)
