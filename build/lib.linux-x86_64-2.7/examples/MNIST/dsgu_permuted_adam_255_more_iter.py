...
def permute_data(train_data,test_data):
    p = numpy.random.permutation(len(train_data[0]))
    return np.array([img[p] for img in train_data]), np.array([img[p] for img in test_data])
...
X_train, X_test = permute_data(X_train, X_test)
...
record_file = FileIO()
record_file.save_pickle(history.val_losses, "dsgu_permuted_adam_%s_255_more_iter_titan_x_losses" % ClassModel.__class__.__name__.lower())
record_file.save_pickle(history.times, "dsgu_permuted_adam_%s_255_more_iter_titan_x_times" % ClassModel.__class__.__name__.lower())
